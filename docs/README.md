...
- **The shift** — one button opens a terminal per registered agent and connects the
  ...
  conversations: the books close with the working day (§8.1).
- **Discovery** — `auto` (any pair may start talking; lines form on first message) or
  `manual` (agents see and reach only whom the operator has linked — sub-teams) (§5.8).
- **The archive** — finished or unlinked conversations move to an immutable archive;
  the board shows only live lines (§5.7).
...
itself: both `make demo-stop` and a re-run remove the previous cast from the board and
delete its throwaway archives — the board looks the way it did before the demo. The
...
agent yourself from a second terminal (`courtyard-puppet --behavior manual` —
`/pending`, `/approve`, `/auto`, `/help`, …).

**Python version.** Pinned in `.python-version` as `3.14` — deliberately minor-only,
not `3.14.x`, so homebrew patch upgrades keep matching the pin instead of fighting it.
