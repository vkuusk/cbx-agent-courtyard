...
| D33 | **The team charter: the team defined as files (item 41, `team-charter.md`)** — one directory per team, operator-chosen, typically the root of its own git repo, never inside an agent's workdir; **files are the source of truth**, the WebUI an editor and viewer that writes back (agent add/edit/remove included); the hub keeps a registry of teams with one current (Admin Teams subpage; switching a running hub postponed); `team-definition.yml` with a single team root maps agents to `agent-config-dir` subdirectories (card.yml plus description/owns/anti-scope prose files); workdirs stay per-machine, never in the shared charter; reload is explicit, the hub never watches the filesystem; hook-loaded context postponed; of the charter only one anti-scope line per peer reaches the envelope | **Accepted** (architect, 2026-09-07 — the full design discussion is `team-charter.md`; his driving use case: team setups shareable between engineers by publishing the charter repo) | Team design becomes reviewable in a pull request and portable between machines; enforced-versus-advisory is classified per item at implementation time |
