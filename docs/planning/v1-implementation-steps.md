...
| 7 — WebUI: quickstart UX (agile) | ⏳ started | 2026-08-20 | **Scope cut (architect, D16):** 6e L1 launch, 6f live mode, and the pi adapter moved to the post-v1 parking lot — v1 is Claude Code only, hub on the host. New step 7 = quickstart-shaped WebUI (7a MainBoard, absorbing Gate + Inbox · 7b Agent Admin · 7c Courtyard Admin, new · further letters from feedback), **agile per page**: design proposal → review → implement → approve. v1 acceptance = the two-agent quickstart run through this UI (operator → `main-admin` → `infra-claude`), doubling as the D14 wake-at-turn-end check. |
...
- ~~6e. Launch L1~~ · ~~6f. Live mode~~ — **moved to the post-v1 parking lot** (architect,
...
(e.g. `main-admin` and `infra-claude`), each in its own terminal.

**The quickstart scenario (= v1 acceptance):**
(a) both agents registered (and installed via 6d);
(b) the operator messages `main-admin` from the WebUI, and `main-admin` asks `infra-claude`
something simple (e.g. list the files in your directory). The same run doubles as the D14
wake-at-turn-end check (design §14 risk 3).

**Working mode: agile, not waterfall** (architect, 2026-08-20). Per page: design proposal →
architect review → implement → approve. No up-front plan beyond the page list; letter-items
are added from architect feedback as we go.
...
- **7a. MainBoard** — the daily page. Absorbs the standalone Gate and Inbox pages (gate
  ...
  destinations).
- **7b. Agent Admin** — registration, install (6d), launch command, removal.
- **7c. Courtyard Admin** — the courtyard itself (new page): housekeeping such as clearing
  dead registrations/lines, defaults such as supervision mode, health.
...
wherever this step's design puts it.
...
| 7 | §10 WebUI consolidated for the quickstart (MainBoard absorbs gate+inbox; Courtyard Admin new); v1 acceptance |
