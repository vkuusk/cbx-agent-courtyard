...
- **6d. Install** (was: invite installer): one file now — the Agents page already prints the
  agent's `.mcp.json`; add a hub-side "write it into the workdir" action for dev mode (merge
  with backup, revert) so the operator never hand-edits. Open: where the token lives
  (inline / `${VAR}` / token file).
- **6e. Launch**: L0 (copy-paste command in UI) + L1 (macOS `osascript` terminal spawn from
  the UI); "connected" flips on the attach handshake. A closed agent with mail waiting shows
  its resume command beside the banner — resume-wake stays an operator action (D14).
